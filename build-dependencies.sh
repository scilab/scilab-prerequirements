...
function download_dependencies() {
    ...
    [ ! -e arpack-ng-$ARPACK_VERSION.tar.gz ] && curl -kL -o arpack-ng-$ARPACK_VERSION.tar.gz https://github.com/opencollab/arpack-ng/archive/$ARPACK_VERSION.tar.gz
    [ ! -e curl-$CURL_VERSION.tar.gz ] && curl -kL -o curl-$CURL_VERSION.tar.gz https://github.com/curl/curl/archive/curl-${CURL_VERSION//\./_}.tar.gz
    [ ! -e eigen-$EIGEN_VERSION.tar.gz ] && curl -kL -o eigen-$EIGEN_VERSION.tar.gz http://bitbucket.org/eigen/eigen/get/$EIGEN_VERSION.tar.gz
    [ ! -e fftw-$FFTW_VERSION.tar.gz ] && curl -kL -o fftw-$FFTW_VERSION.tar.gz http://www.fftw.org/fftw-$FFTW_VERSION.tar.gz
    [ ! -e hdf5-$HDF5_VERSION.tar.gz ] && curl -kL -o hdf5-$HDF5_VERSION.tar.gz https://support.hdfgroup.org/ftp/HDF5/releases/hdf5-1.8/hdf5-$HDF5_VERSION/src/hdf5-$HDF5_VERSION.tar.gz
    [ ! -e libxml2-$LIBXML2_VERSION.tar.gz ] && curl -kL -o libxml2-$LIBXML2_VERSION.tar.gz http://xmlsoft.org/sources/libxml2-$LIBXML2_VERSION.tar.gz
    [ ! -e matio-$MATIO_VERSION.tar.gz ] && curl -kL -o matio-$MATIO_VERSION.tar.gz http://downloads.sourceforge.net/project/matio/matio/$MATIO_VERSION/matio-$MATIO_VERSION.tar.gz
    [ ! -e ocaml-$OCAML_VERSION.tar.gz ] && curl -kL -o ocaml-$OCAML_VERSION.tar.gz http://caml.inria.fr/pub/distrib/ocaml-4.01/ocaml-$OCAML_VERSION.tar.gz
    [ ! -e openssl-$OPENSSL_VERSION.tar.gz ] && curl -kL -o openssl-$OPENSSL_VERSION.tar.gz http://www.openssl.org/source/openssl-$OPENSSL_VERSION.tar.gz
    [ ! -e openssh-$OPENSSH_VERSION.tar.gz ] && curl -kL -o openssh-$OPENSSH_VERSION.tar.gz http://mirrors.ircam.fr/pub/OpenBSD/OpenSSH/portable/openssh-$OPENSSH_VERSION.tar.gz
    [ ! -e SuiteSparse-$SUITESPARSE_VERSION.tar.gz ] && curl -kL -o SuiteSparse-$SUITESPARSE_VERSION.tar.gz http://faculty.cse.tamu.edu/davis/SuiteSparse/SuiteSparse-$SUITESPARSE_VERSION.tar.gz
    [ ! -e pcre-$PCRE_VERSION.tar.gz ] && curl -kL -o pcre-$PCRE_VERSION.tar.gz http://ftp.pcre.org/pub/pcre/pcre-$PCRE_VERSION.tar.gz
    [ ! -e tcl$TCL_VERSION-src.tar.gz ] && curl -kL -o tcl$TCL_VERSION-src.tar.gz http://prdownloads.sourceforge.net/tcl/tcl$TCL_VERSION-src.tar.gz
    [ ! -e tk$TK_VERSION-src.tar.gz ] && curl -kL -o tk$TK_VERSION-src.tar.gz http://prdownloads.sourceforge.net/tcl/tk$TK_VERSION-src.tar.gz
    [ ! -e zlib-$ZLIB_VERSION.tar.gz ] && curl -kL -o zlib-$ZLIB_VERSION.tar.gz http://downloads.sourceforge.net/project/libpng/zlib/$ZLIB_VERSION/zlib-$ZLIB_VERSION.tar.gz
    [ ! -e libpng-$PNG_VERSION.tar.gz ] && curl -kL -o libpng-$PNG_VERSION.tar.gz http://prdownloads.sourceforge.net/libpng/libpng-$PNG_VERSION.tar.gz
    [ ! -e gluegen-v$JOGL_VERSION.tar.7z ] && curl -kL -o gluegen-v$JOGL_VERSION.tar.7z http://jogamp.org/deployment/archive/rc/v$JOGL_VERSION/archive/Sources/gluegen-v$JOGL_VERSION.tar.7z
    [ ! -e jogl-v$JOGL_VERSION.tar.7z ] && curl -kL -o jogl-v$JOGL_VERSION.tar.7z http://jogamp.org/deployment/archive/rc/v$JOGL_VERSION/archive/Sources/jogl-v$JOGL_VERSION.tar.7z

    # xmlgraphics-commons is included within FOP
    # Batik is included within FOP
    [ ! -e fop-$FOP_VERSION-bin.zip ] && curl -kL -o fop-$FOP_VERSION-bin.zip http://wwwftp.ciril.fr/pub/apache/xmlgraphics/fop/binaries/fop-$FOP_VERSION-bin.zip
}
